...
/*
🔹 What is tryLock()?
   tryLock() is a method of ReentrantLock that tries to acquire a lock without waiting.
   If the lock is available → it takes it
   If not → it immediately returns false (no waiting)

   🔹 What is unlock()?
      unlock() is used to release the lock that was acquired using lock() or tryLock().
